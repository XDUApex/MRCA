...
def causal_anlysis(services_folder, front_service_path,times_file,ground_truth_path,xxx):
    ...
    print("*TEST* service_folder ", services_folder)
    print("*TEST* front_service_path ", front_service_path)
    print("*TEST* times_file ", times_file)
    print("*TEST* ground_truth_path ", ground_truth_path)
    print("*TEST* xxx ", xxx)
        ...
        for file in all_files:
            ...
            if 'Time' in data.columns:
                ...
                if all(col in data.columns for col in required_columns):
                    ...
                    # print(f"*TEST* Filtered data for service {service_name}: \n", filtered_data.head())
    ...
    def perform_causality_test(df, front_service_metrics, other_service_metrics, max_lag=2):
        ...
        # 检查前端指标，过滤掉常数列
        for front_metric in front_service_metrics:
            if front_metric in df.columns:
                data = df[front_metric].dropna()
                is_constant = data.std() < 1e-10 or data.nunique() <= 1
                if not is_constant:
                    valid_front_metrics.append(front_metric)
        
        # 检查后端指标，过滤掉常数列
        for other_metric in other_service_metrics:
            if other_metric in df.columns:
                data = df[other_metric].dropna()
                is_constant = data.std() < 1e-10 or data.nunique() <= 1
                if not is_constant:
                    valid_other_metrics.append(other_metric)
        
        # 只对有效的（非常数）指标进行Granger检验
